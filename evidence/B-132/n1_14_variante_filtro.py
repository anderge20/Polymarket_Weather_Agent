...
if _os.environ.get("OBS_CORREGIDA")=="1":
    _full={dt.date.fromisoformat(k): v for k,v in _json.load(open(_os.environ["FULL_JSON"])).items()}
    _cambian=sum(1 for d in obs if d in _full and _full[d]!=obs[d]); _falta=sum(1 for d in obs if d not in _full)
    obs={d:(_full[d] if d in _full else v) for d,v in obs.items()}
    print(f"[OBS CORREGIDA] dias {len(obs)}  cambian {_cambian}  sin serie completa (se dejan) {_falta}")
...
_modo=_os.environ.get("FILTRO_EVENTOS","todos")
def _particiona(bs):
    bs=sorted(bs, key=lambda b: (-1e9 if b[0] is None else b[0]))
    if bs[0][0] is not None or bs[-1][1] is not None: return False
    return all(a[1] is not None and b[0] is not None and b[0]==a[1]+1 for a,b in zip(bs,bs[1:]))
if _modo=="un_ganador":
    eventos={td:v for td,v in eventos.items() if sum(w for _,_,w in v)==1}
elif _modo=="particion":
    eventos={td:v for td,v in eventos.items() if _particiona([(lo,hi) for lo,hi,_ in v]) and sum(w for _,_,w in v)==1}
print(f"[FILTRO {_modo}] eventos tras filtro: {len(eventos)}")
