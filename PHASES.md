...
| 2E | Ancla temporal `T = endDate − lead_hours` (decisión D1 de 2E) + registro de estaciones (coordenadas D1-COORD, ICAO variable en el tiempo, tz) | `PHASE_2E_LEAD_HOURS_ANCHOR.md` (RATIFIED / DOCUMENTED, sin código) · registro de estaciones: R7/R27 | DOCUMENTED (ancla) · PENDING (registro de estaciones) |
...
1. **Fases**
   * **2E = ancla temporal + registro de estaciones.** El único documento 2E del repo es
     `PHASE_2E_LEAD_HOURS_ANCHOR.md`. El borrador `PHASE_2E_SETTLEMENT_OPERATOR_DESIGN.md`
     (untracked, fuera de git) NO es 2E: el `SettlementOperator` pertenece a R12 y se
     renombrará al integrarse (no a 2E).
