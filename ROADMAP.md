...
| R23/R24 | **DISEÑO FIJADO POR EL USUARIO (A-29)**: modo papel sobre **GitHub Actions** (cron, reanudable, estado versionado); Hetzner sólo si Actions no da y sin coste; Mac última opción | A-29 |
| Cuota | **SIN clave de pago (A-29)**: captura desde el borde del archivo hacia el presente; se aceptan las pérdidas de cola | A-28, A-29 |
| Proceso | **Nada se declara VALIDADO sin refutación hostil previa (A-29.4)**; presunción invertida; Codex con objeción retroactiva al volver | A-29 |

## 7. Delta 2026-09-09 — pista A tras el reparto de B-8 (A-31)

| Ítem | Cambio de estado | Evidencia |
|---|---|---|
| **R22** | **HECHO** — `collector.py`: polling REST del book en lotes (`POST /books`), sesiones = ciclos reanudables, parada limpia ante 429. **`CLOB_WS_URL` sigue UNVERIFIED y la política de polling queda ADOPTADA**: Actions no sostiene un proceso entre disparos de cron (A-29.2), así que el websocket es discutible pero irrelevante para el host que el usuario eligió. Verificado en vivo read-only: **1.122 tokens en 23 peticiones**, y una reanudación del mismo ciclo cuesta **0 peticiones** | `feat/paper-actions`, `tests/test_collector.py` (27) |
| **Persistencia** | **NUEVO** — `store.py`: shards NDJSON append-only en una rama de datos. Resuelve dónde vive el estado entre ejecuciones efímeras sin coste ni credenciales; los shards nunca se reescriben (dos ciclos no pueden colisionar) y la DuckDB pasa a ser estado **derivado**, reconstruible, fuera de git | `tests/test_store.py` (22) |
| **R23** | **HECHO (motor)** — `paper.py`: fill simulado caminando el book observado, VWAP, fees de D19, `price_layer='SIMULATED_EXECUTABLE'`. La puerta de edge neto se evalúa **al precio alcanzable, no al cotizado**. FADE = compra taker del token complementario **contra su propio book**, nunca `1 − ask`. **Gate D0 estructural**: sin wallet, sin firma, sin ruta de orden, y los tests lo verifican sobre el texto fuente | `tests/test_paper.py` (45) |
| **Orquestación** | **NUEVO** — `scripts/paper_cycle.py` + 2 workflows de Actions (colector cada 3 h; ciclo diario a 11:40Z y 02:40Z, uno por lead). Etapas con estado propio: lo que no tiene sustrato reporta SKIPPED con la razón, nunca inventa una entrada | `tests/test_paper_cycle.py` (20) |
| **R24** | **BORRADOR EN REFUTACIÓN** — `PREREG_PAPER_RUN.md`. **No congelado** (A-29.4) | workflow `wf_edf49ca7-104` |
| **M2 de B** | **REFUTADO 3/3** — 22 hallazgos; verificados por A: la clave de emparejamiento no existe como columna y el join depende del `TimeZone` de sesión (**1.881–2.599 pares**, B declara 2.595); `lead` no es columna; 0 apariciones de `T_asof`/`T_end`/`endDate` en el documento; n≥30 sólo en **4/98** grupos (n≥20 en 88/98) | A-32 |

**Dos defectos propios encontrados al preregistrar R24 y corregidos** (ninguno podía saltar en un ciclo `collect-only`, que es todo lo que los smoke tests habían ejercitado):
1. la búsqueda de fees consultaba `market_fee_schedule WHERE market_id`, y esa tabla **no tiene** `market_id` → error de binder en la primera señal accionable; se une por `markets.fee_regime`;
